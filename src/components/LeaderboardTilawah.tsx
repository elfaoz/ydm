...
interface LeaderboardTilawahProps {
  activityRecords: ActivityRecord[];
}

const LeaderboardTilawah: React.FC<LeaderboardTilawahProps> = ({ activityRecords }) => {
  const studentActivities = activityRecords.reduce((acc: any[], record) => {
    const completed = record.activities['tilawah'] ? 1 : 0;
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-emerald-50 to-green-50">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-lg font-semibold text-gray-800">Leaderboard Aktivitas Tilawah 📖</h3>
        </div>
      </div>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {leaderboard.length > 0 ? leaderboard.map((student, index) => (
              <tr key={student.nama} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-center">
                  <div className="text-sm font-bold text-emerald-600">{student.totalAktivitas} hari</div>
                </td>
              </tr>
            )) : (
              <tr>
...
export default LeaderboardTilawah;
